...
public class Vente extends FactureCF {
    ...
    public void payerAvecTTC(String u, Connection c, double montantTtcForce, double totalReversed) throws Exception {
        ...
        // Ne plus annuler totalement l'encaissement puis recréditer la totalité.
        // On ne poste que l'écart (delta) entre le nouveau TTC et l'ancien TTC.
        double delta = montantTtcForce - totalReversed;

        if (Math.abs(delta) < 0.0001) {
            // Pas de mouvement si montants identiques
            return;
        }
        ...
        if (delta > 0) {
            // Complément à encaisser (entrée de caisse)
            mvt.setCredit(delta);
            mvt.setDesignation("Complément encaissement (changement facture) : " + this.getId());
        } else {
            // Remboursement au client (sortie de caisse)
            mvt.setDebit(-delta);
            mvt.setDesignation("Remboursement différence (changement facture) : " + this.getId());
        }
