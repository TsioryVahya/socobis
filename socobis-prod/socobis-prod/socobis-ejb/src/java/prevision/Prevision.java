...
public class Prevision extends MvtCaisse{
    ...
    public ClassMAPTable createObject(String u, Connection c) throws Exception{
        // Option A: convertir et stocker en AR
        try {
            String idDev = this.getIdDevise();
            if (idDev != null && idDev.compareToIgnoreCase("AR") != 0) {
                // Récupérer le dernier taux à la date de la prévision (ou à défaut aujourd'hui)
                String datyStr = (this.getDaty() != null) ? Utilitaire.formatterDaty(this.getDaty()) : null;
                double taux = this.getTaux();
                if (taux == 1) {
                    taux = TauxDeChange.getLastTaux(c, datyStr, idDev);
                }
                // Convertir les montants en AR
                double d = this.getDebit();
                double cr = this.getCredit();
                if (d > 0) this.setDebit(d * taux);
                if (cr > 0) this.setCredit(cr * taux);
                // Forcer AR et taux=1 pour stockage clair en devise de base
                this.setIdDevise("AR");
                this.setTaux(1);
            } else {
                // Devise AR: s'assurer que le taux = 1
                this.setTaux(1);
            }
        } catch (Exception ex) {
            // Laisser remonter après journalisation éventuelle
            throw ex;
        }
