...
public class MvtCaisse extends ClassEtat{
    ...
    public MvtCaissePrevision[] attacherPrevision(PrevisionComplet[] listePrevision ,String u,Connection c)throws Exception {
        ...
        for(PrevisionComplet prevision : listePrevision){
            ...
            // Affecter le montant du bon côté selon le sens du mouvement
            if(this.isDepense()){
                attachement.setMontantMere(valeur);
            } else {
                attachement.setMontantCredit(valeur);
            }
    ...
    public MvtCaisse attacherPrevision(String idPrevision ,String u,Connection c)throws Exception {
        ...
        // Renseigner la prévision cible sur le mouvement, puis créer la liaison dans la table de jointure
        mvtCaisseBase[0].setIdPrevision(idPrevision);
        mvtCaisseBase[0].updateToTableWithHisto(u, c);
        // Charger la prévision complète pour construire la liaison avec le bon sens et montant
        PrevisionComplet pc = new PrevisionComplet();
        pc.setId(idPrevision);
        PrevisionComplet[] pcs = (PrevisionComplet[]) CGenUtil.rechercher(pc, null, null, c, " and id='"+idPrevision+"'");
        if(pcs!=null && pcs.length>0){
            MvtCaissePrevision[] liens = mvtCaisseBase[0].attacherPrevision(new PrevisionComplet[]{pcs[0]}, u, c);
            for(MvtCaissePrevision l:liens){ l.createObject(u, c); }
        }
        return mvtCaisseBase[0];
    }
    
    public MvtCaisse attacherPrevision(String u ,Connection c)throws Exception {
        // Attacher au(x) enregistrement(s) de prévision renseigné(s) sur le mouvement
        if(this.getIdPrevision()==null || this.getIdPrevision().trim().isEmpty()){
            throw new Exception("Aucune prévision sélectionnée");
        }
        this.updateToTableWithHisto(u, c);
        PrevisionComplet pc = new PrevisionComplet();
        pc.setId(this.getIdPrevision());
        PrevisionComplet[] pcs = (PrevisionComplet[]) CGenUtil.rechercher(pc, null, null, c, " and id='"+this.getIdPrevision()+"'");
        if(pcs==null || pcs.length==0){
            throw new Exception("Prévision introuvable");
        }
        MvtCaissePrevision[] liens = this.attacherPrevision(new PrevisionComplet[]{pcs[0]}, u, c);
        for(MvtCaissePrevision l:liens){ l.createObject(u, c); }
        return this;
    }
    ...
    {
        // Recherche robuste des prévisions liées à l'origine (vente/facture) via IDFACTURE,
        // en tenant compte du sens (dépense/recette) et des vues/tables disponibles.
        String sensFilter = "";
        try{
            if(this.isDepense()) sensFilter = " and DEBIT > 0 ";
            else if(this.isRecette()) sensFilter = " and CREDIT > 0 ";
        }catch(Exception e){ /* ignore */ }

        // Certaines origines peuvent contenir plusieurs IDs séparés par ';;'
        String[] origs = (this.getIdOrigine()!=null && this.getIdOrigine().trim().length()>0)
                ? this.getIdOrigine().split(";;") : new String[0];
        String aWhereIdFact = origs.length>0
                ? (" and IDFACTURE in ("+Utilitaire.tabToString(origs, "'", ",")+") ")
                : (this.getIdOrigine()!=null ? (" and IDFACTURE='"+this.getIdOrigine()+"' ") : "");

        // 1) Essayer sur PREVISION_COMPLET_CPLPOSITIF si demandé
        if(nomT!=null && nomT.trim().length()>0){
            PrevisionComplet prevision = new PrevisionComplet();
            prevision.setNomTable(nomT);
            PrevisionComplet[] res = (PrevisionComplet[]) CGenUtil.rechercher(prevision, null, null, c,
                    aWhereIdFact + sensFilter + " order by daty ASC");
            if(res!=null && res.length>0) return res;
        }

        // 2) Essayer la table/view par défaut de PrevisionComplet (PREVISION_COMPLET_CPL)
        {
            PrevisionComplet prevision = new PrevisionComplet();
            PrevisionComplet[] res = (PrevisionComplet[]) CGenUtil.rechercher(prevision, null, null, c,
                    aWhereIdFact + sensFilter + " order by daty ASC");
        ...
        // 3) Dernier recours: table PREVISION (base) si disponible
        {
            PrevisionComplet prevision = new PrevisionComplet();
            prevision.setNomTable("PREVISION");
            PrevisionComplet[] res = (PrevisionComplet[]) CGenUtil.rechercher(prevision, null, null, c,
                    aWhereIdFact + sensFilter + " order by daty ASC");
            if(res!=null && res.length>0) return res;
        }

        return new PrevisionComplet[0];
    }
    ...
    public ClassMAPTable validerObject(String u, Connection c) throws Exception{
        // Harmoniser le taux multi-devise: utiliser le taux de la Prévision liée (et non plus la date de vente/facture)
        if(this.getIdOrigine()!=null && this.getIdOrigine().startsWith("VNT")){
            try{
                double tauxPrev = change.TauxDeChange.getTauxPrevision(c, this.getIdOrigine());
                if(tauxPrev > 0){ this.setTaux(tauxPrev); }
                // Si aucune devise fournie, essayer de reprendre la devise de la prévision liée
                if(this.getIdDevise()==null || this.getIdDevise().trim().isEmpty()){
                    prevision.PrevisionComplet[] pcs = this.getPrevisionLie("", c);
                    if(pcs!=null && pcs.length>0 && pcs[0].getIdDevise()!=null && pcs[0].getIdDevise().trim().length()>0){
                        this.setIdDevise(pcs[0].getIdDevise());
                    }
                }
            }catch(Exception ignore){ }
        } else if(this.getIdOrigine()!=null && this.getIdOrigine().startsWith("FCF")){
            try{
                double tauxPrev = change.TauxDeChange.getTauxPrevision(c, this.getIdOrigine());
                if(tauxPrev > 0){ this.setTaux(tauxPrev); }
                if(this.getIdDevise()==null || this.getIdDevise().trim().isEmpty()){
            ...
            // Encaissement hors vente: appliquer le dernier taux connu si non fourni
            try{
                String dev = this.getIdDevise();
                double tx = this.getTaux();
                if(dev!=null && dev.trim().length()>0){
                    // Si aucun taux ou taux par défaut (1) pour devise étrangère, charger le dernier taux
                    if(tx<=0 || (tx==1 && !"AR".equalsIgnoreCase(dev))){
                        double last = change.TauxDeChange.getLastTaux(c, utilitaire.Utilitaire.dateDuJour(), dev);
                        if(last>0){ this.setTaux(last); }
                    }
                }
            }catch(Exception ignore){ }
        }

        // Normaliser en Ariary: stocker les montants en AR dans MOUVEMENTCAISSE
        try{
            String dev = this.getIdDevise();
            double tx = this.getTaux();
            if(dev!=null && !dev.equalsIgnoreCase("AR")){
                double cr = 0; double db = 0;
                try{ cr = this.getCredit(); }catch(Exception e){}
                try{ db = this.getDebit(); }catch(Exception e){}
                if(cr > 0 && tx > 0){ this.setCredit(cr * tx); }
                if(db > 0 && tx > 0){ this.setDebit(db * tx); }
                // Basculer la devise et le taux pour refléter un stockage en Ariary
                this.setIdDevise("AR");
                this.setTaux(1);
            }
        }catch(Exception ignore){ }
        ...
        if(this.getIdOrigine()!=null&&this.getIdOrigine().compareToIgnoreCase("")!=0&&this.getIdOrigine().startsWith("VNT"))
        {
            ...
            // Attacher automatiquement aux prévisions de la vente (recettes)
        ...
        else if(this.getIdOrigine()!=null&&this.getIdOrigine().compareToIgnoreCase("")!=0&&this.getIdOrigine().startsWith("FCF"))
        {
            String [] ids = this.getIdOrigine().split(";;");
            LiaisonPaiement li = new LiaisonPaiement();
            LiaisonPaiement[] bls = (LiaisonPaiement[]) CGenUtil.rechercher(li, null, null,c, " and id2 in ("+Utilitaire.tabToString(ids, "'", ",")+" ) ");
            ...
            // Attacher automatiquement aux prévisions de la facture fournisseur (dépenses)
            PrevisionComplet[] previsionComplets = this.getPrevisionLie("", c);
            MvtCaissePrevision[] lienMvtPrev = this.attacherPrevision(previsionComplets, u, c);
            for(MvtCaissePrevision mvt:lienMvtPrev)
            {
                mvt.createObject(u, c);
            }
        } else if (this.getIdPrevision()!=null && this.getIdPrevision().trim().length()>0){
            // Rattachement explicite à une Prévision sélectionnée dans le formulaire
            PrevisionComplet pc = this.getPrevision("", c);
            boolean mvtDep = this.isDepense();
            if(pc!=null){
                boolean prevOk = mvtDep ? (pc.getDebit() > 0) : (pc.getCredit() > 0);
                if(!prevOk){
                    if(mvtDep){
                        throw new Exception("La prévision sélectionnée n'a pas de DÉPENSE (Débit>0) alors que le mouvement est une DÉPENSE. Choisissez une prévision avec Débit.");
                    } else {
                        throw new Exception("La prévision sélectionnée n'a pas de RECETTE (Crédit>0) alors que le mouvement est une RECETTE. Choisissez une prévision avec Crédit.");
                    }
                }
                MvtCaissePrevision[] liens = this.attacherPrevision(new PrevisionComplet[]{pc}, u, c);
                for(MvtCaissePrevision l : liens){
                    l.createObject(u, c);
                }
            }
        }
